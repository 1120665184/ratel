...
public class AgentScopeKnowledgePageMergeModelClient implements KnowledgePageMergeModelClient {
    ...
    public AgentScopeKnowledgePageMergeModelClient(AgentScopeResponseParser responseParser) {
        ...
        try {
            ...
            try (ReActAgent agent = ReActAgent.builder()
                ...
                List<Msg> messages = List.of(Msg.builder()
                        ...
                        .textContent(prompt)
                        .build());
                T response = responseParser.parse(
                        agent.call(messages, responseType, RuntimeContext.builder().build()).block(),
                        responseType);
                if (response == null) {
                    throw new BusinessException(KitErrorCode.E03008);
                }

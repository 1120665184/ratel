...
public class DatabaseSearchAgent {
    ...
    private String buildSkillContent() {
        ...
        sb.append("3. 对每个需要查询的表，调用 `get_table_detail` 获取字段详情和权限\n");
        sb.append("4. 基于字段信息生成SELECT语句（只包含有权限的字段）\n");
        sb.append("5. 调用 `execute_sql` 执行SQL，传入正确的所属服务和数据源\n");
    ...
    private String buildSystemPrompt() {
        return """
                ...
                - **获取表结构信息**：你可以通过 `get_table_detail` 工具获取指定表的字段详细信息（包括字段名、类型、注释及当前用户是否有该字段的查询权限）。
                - **执行SQL查询**：你可以通过 `execute_sql` 工具执行只读的SQL语句（仅限SELECT），并返回查询结果。
                ...
                - **必须先获取表结构**：在生成SQL之前，你必须先调用 `get_table_detail` 获取当前可用的表字段信息，确保生成的SQL正确且高效。
                ...
                4. **获取字段详情**：对每个涉及的表调用 `get_table_detail` 获取字段名称、类型、注释及权限信息。
                5. **编写 SELECT SQL**：
                   - 基于获取的真实表名和字段名，构造 `SELECT` 语句，**必须列出具体字段名**。
                   - 只包含当前用户有查询权限的字段。
                   - 可以包含 `WHERE`、`GROUP BY`、`HAVING`、`ORDER BY`、`LIMIT` 等子句，但绝不能有修改操作。
                6. **执行查询**：调用 `execute_sql` 执行SQL，传入正确的所属服务和数据源。
                ...
                **你的响应步骤**：
                ...
                3. 生成SQL：
                   ...
                   ```
                4. 调用 `execute_sql(modulePrefix="order", dataSource="master", sql="...")` 执行。
                5. 格式化返回结果。
                """;
    }

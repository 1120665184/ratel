...
const AccountBindSection: React.FC<AccountBindSectionProps> = ({userId, accounts, onRefresh}) => {
    ...
    const handleBind = (_identityType: string) => async (data: SysAccountBindDTO) => {
        try {
    ...
    return (
        <div>
            <div style={{fontSize: 13, fontWeight: 600, marginBottom: 10}}>账号绑定</div>
            <div style={{display: 'flex', flexDirection: 'column', gap: 8}}>
                {allTypes.map((type) => {
                    const account = accountMap.get(type);
                    const typeInfo = IDENTITY_TYPE_MAP[type] || {label: type, icon: '🔑'};
                        ...
                        >
                            <div style={{
                                ...
                                <div style={{display: 'flex', alignItems: 'center', gap: 8}}>
                                    ...
                                    <div>
                                        <div style={{fontSize: 12, fontWeight: 500}}>{typeInfo.label}</div>

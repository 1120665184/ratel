...
import {
  ...
  PlayCircleOutlined, PauseCircleOutlined, ThunderboltOutlined, FileSearchOutlined,
...
import { PERM_ADD, PERM_EDIT, PERM_REMOVE, PERM_START, PERM_TRIGGER, PERM_LOG } from './permissionConstants';
...
const JobPage: React.FC = () => {
  const {
    ...
    ensureInitialized, handlePageChange,
    handleAdd, handleUpdate, handleRemove,
    handleStart, handleStop, handleTrigger,
  } = useJob();
  ...
  const handleSave = useCallback(async (data: JobInfoCreateDTO) => {
  ...
  const getMoreItems = (record: JobInfo): MenuProps['items'] => {
    ...
    if (record.triggerStatus === 0 && canStart) {
      items.push({ key: 'start', icon: <PlayCircleOutlined />, label: '启动', onClick: () => handleStart(record.id!) });
    }
    if (record.triggerStatus === 1 && canStart) {
      items.push({ key: 'stop', icon: <PauseCircleOutlined />, label: '停止', onClick: () => handleStop(record.id!) });
    }
    if (canTrigger) {
      items.push({ key: 'trigger', icon: <ThunderboltOutlined />, label: '立即执行', onClick: () => handleOpenTrigger(record) });
    }
    if (canLog) {
      items.push({ key: 'log', icon: <FileSearchOutlined />, label: '调度日志', onClick: () => handleViewLog(record) });
    }
    if (record.glueType !== 'BEAN' && canEdit) {
      items.push({ key: 'glue', icon: <CodeOutlined />, label: 'GLUE编辑', onClick: () => handleGlueEdit(record) });
    }
    if (canRemove) {
      items.push({ key: 'remove', icon: <DeleteOutlined />, label: '删除', danger: true, onClick: () => { handleRemove(record.id!); } });
    }
    return items;
  };
  ...
  const columns: TableProps<JobInfo>['columns'] = [
    {
      ...
      render: (_: unknown, __: JobInfo, index: number) => (currentPage - 1) * pageSize + index + 1,
    },
    { title: '任务名称', dataIndex: 'name', width: 160, ellipsis: true },
    { title: '负责人', dataIndex: 'author', width: 100 },
    {
      title: '任务类型', width: 100,
      render: (_: unknown, record: JobInfo) => {
        const mode = deriveJobMode(record);
        const colorMap: Record<string, string> = { URL: 'blue', BEAN: 'green', GLUE: 'orange' };
        const labelMap: Record<string, string> = { URL: '平台URL', BEAN: 'BEAN', GLUE: 'GLUE' };
        return <Tag color={colorMap[mode]}>{labelMap[mode]}</Tag>;
      },
    },
    { title: 'Handler', dataIndex: 'executorHandler', width: 140, ellipsis: true },
    {
      title: '调度类型', width: 100,
      render: (_: unknown, record: JobInfo) => {
        const map: Record<string, string> = { NONE: '无', CRON: 'Cron', FIX_RATE: '固定速率' };
        return map[record.scheduleType] ?? record.scheduleType;
      },
    },
    { title: '调度配置', dataIndex: 'scheduleConf', width: 150, ellipsis: true },
    {
      title: '状态', dataIndex: 'triggerStatus', width: 100,
      render: (val: number) => <Tag color={val === 1 ? 'green' : 'red'}>{val === 1 ? '运行中' : '已停止'}</Tag>,
    },
    {
    ...
    <div className={styles.jobPage}>
        ...
        <Form form={searchForm} layout="inline" component={false}>
          <div className={styles.searchItem}>
            ...
            <Form.Item name="name" noStyle>
              <Input placeholder="请输入" allowClear style={{ width: 160 }} onPressEnter={handleSearch} />
            </Form.Item>
          </div>
          <div className={styles.searchItem}>
          ...
          <div className={styles.searchItem}>
            ...
            <Form.Item name="triggerStatus" noStyle>
              <Select placeholder="全部" allowClear style={{ width: 120 }} options={[...TRIGGER_STATUS_OPTIONS]} />
            </Form.Item>
          </div>
        </Form>
        ...
        <div className={styles.tableHeader}>
          ...
          <Space>
            <AuthGate buttonKey={PERM_ADD}>
              <Button type="primary" icon={<PlusOutlined />} onClick={handleCreate}>新增任务</Button>
            </AuthGate>
            {canLog && (
              <Button icon={<DeleteOutlined />} onClick={() => setClearLogVisible(true)} data-ai-approval>
                清理日志
              </Button>
            )}
          </Space>
      ...
      <GlueEditorModal
        ...
        onSave={async (_type, source, remark) => {
          ...
          const success = await handleUpdate(buildGlueUpdatePayload(glueRecord, _type, source, remark));
          if (success) {

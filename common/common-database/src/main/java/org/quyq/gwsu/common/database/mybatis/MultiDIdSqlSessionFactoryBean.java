package org.quyq.gwsu.common.database.mybatis;
...
import org.quyq.gwsu.common.database.mybatis.builder.xml.XMLMapperBuilder;
 ...
 * @description
 */
public class MultiDIdSqlSessionFactoryBean extends MybatisSqlSessionFactoryBean {
        ...
        if (this.mapperLocations != null) {
            if (this.mapperLocations.length == 0) {
                ...
                for (Resource mapperLocation : this.mapperLocations) {
                    ...
                    try {
                        //自定义xml解析器，主要用于xml配置有多databaseId方法时，支持启动时加载所有databaseId的方法（mybatis原油逻辑智能加载启动时当前数据源的方法，其他databaseId配置方法会忽略加载）
                        // 加载所有databaseId的目的是运行时数据源切换，可以动态适配不同数据源所属数据库类型的sql执行。
